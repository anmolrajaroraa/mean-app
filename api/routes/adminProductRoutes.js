...
const productCrud=require('../../db/crudOperations/adminProduct'); 

adminProductRoutes.post('/upload',function(req,res){
...
adminProductRoutes.get('/getProducts',verifyToken,(req,res)=>{
...
adminProductRoutes.post('/editProducts',verifyToken,(req,res)=>{
    jwt.verify(req.token,securekey,(error,authData)=>{
        if(error){
            res.json("token not valid or session timed out");
        }
...
adminProductRoutes.post('/editCategoryList',verifyToken,(req,res)=>{
    jwt.verify(req.token,securekey,(error,authData)=>{
        if(error){
            res.json("token not valid or session timed out");
        }
...
adminProductRoutes.post('imageUpload',(req,res)=>{
...
adminProductRoutes.post('/login',function(req,res){
    var adminData=productCrud.login(res,{id:req.body.id, name:req.body.name, password:req.body.password});
